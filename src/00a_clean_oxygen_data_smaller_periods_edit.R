...
df <- readRDS(file.path("data", "hourly_inputs.RDS"))
dfq <- readRDS(file.path("data", "daily_Q_inputs.RDS"))

dfall <- mutate(df, date = date(solar.time)) %>%
  left_join(dfq)

dfall <- dfall %>%
  group_by(site, pos) %>%
  mutate(year = year(date),
         period = ntile(year, (max(year) - min(year)) / 5))

dfhr <- ungroup(dfall) %>%
  select(-year, -date, -discharge.daily)

dfqd <- ungroup(dfall) %>%
  select(site, pos, period, date, discharge.daily) %>%
  distinct()

saveRDS(dfhr,  file.path("data", "hourly_inputs_smaller_pds.RDS"))
saveRDS(dfqd,  file.path("data", "daily_q_inputs_smaller_pds.RDS"))

w = unique(ungroup(dfall)[, c("site", "pos", "period")])
nrow(w)
dfall <- ungroup(dfall)
dfall[dfall$site == w$site[3] & dfall$pos == w$pos[3] &
        dfall$period == w$period[3],]
...
x = split(df, ~cut(df$solar.time, 5) + df$site + df$pos)
y = split(dfq, ~cut(dfq$date, 5) + dfq$site + dfq$pos)
...
y = unsplit(x)
x[1]
y = dplyr::filter(df_DO, site == "belleville", pos == "up", period == 1)
seq.along(x)
p = t(sapply(split(df_DO, cut(df_DO$datetime, 5), drop = TRUE)))
