...
df_test <- df_mods |>
  mutate(
    ...
    # Z score and p-value for if NEP slope is 0.04
...
df_up <- left_join(df_dep, df_test) |>
  ...
  # filter(r2_dic > 0.66) |>
  # Ignore heterotrophic source days...can't be
  ...
  mutate(
    ...
    source = case_when(
      ...
      depleted > 6 & p_hco3dic > 0.05  & p_dic0.5 < 0.05 ~ "HCO3",
      depleted > 6 & p_hco3dic < 0.05  & p_dic0.5 < 0.05 & p_spc0.04 > 0.05 ~ "HCO3",
      depleted > 6 & p_hco3dic < 0.05  & p_dic0.5 < 0.05 & p_spc0.04 < 0.05 & 
        between(slo_hco3, -1.2, -0.8) & between(slo_spc, -0.06, -0.03)~ "HCO3",
      depleted > 6 & p_dic0.5 > 0.05 & r2_dic > 0.7 ~ "CaCO3",
      depleted > 6 & p_dic0.5 > 0.05 & r2_dic <= 0.7 & between(slo_spc, -0.2, -0.04) ~ "CaCO3",
      depleted > 6 & between(slo_spc, -0.2, -0.06) & between(slo_hco3, -0.7, -0.4) ~ "CaCO3",
      TRUE ~ "CO2"
    )) 
