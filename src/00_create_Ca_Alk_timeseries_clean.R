...
df_o2 <- df_o2 |>
  ...
  mutate(K600 = slider::slide_dbl(K600, mean, .before = 3*24, na.rm = T),
         KO2 = slider::slide_dbl(KO2, mean, .before = 3*24, na.rm = T),
         depth = slider::slide_dbl(depth, mean, .before = 3*24, na.rm = T),
         NEP = ((O2_uM - lag(O2_uM)) - (KO2 / 24) * 
                  (O2sat_uM - O2_uM)) * depth)
...
# Simple model just based on SpC
lmmod2 <- lm(AT ~ SpC*monthf, data = df_mod_AT)
summary(lmmod2)

# Do predictions
df_pred$Ca_rf <- predict(rf_model_ca, newdata = df_pred)
df_pred <- df_pred |>
  group_by(date) |>
  mutate(Ca_rf = mean(Ca_rf, na.rm = T),
...
# Get predictions
df_pred$ATreg <- predict(AT_mod_all, newdata = df_pred)
...
saveRDS(df_final, file.path("data", "03_CO2", "hourly_pH_SpC_AT_Ca.RDS"))
