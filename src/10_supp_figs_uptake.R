...
# Check for uptake 
df_test <- df_mods |>
  left_join(distinct(df, date, year)) |>
  left_join(distinct(df_tf, date, trophlux)) |>
  ...
  mutate(Z_hco3dic = abs((slo_hco - slo_dic)) / sqrt(se_hco^2 + se_dic^2),
         ...
         Z_nep0.13 = abs((slo_nep - -0.13)) / sqrt(se_nep^2),
         p_nep0.13 = pt(Z_nep0.13, 23, lower.tail = FALSE),
         Z_nep0.17 = abs((slo_nep - -0.17)) / sqrt(se_nep^2),
         p_nep0.17 = pt(Z_nep0.17, 23, lower.tail = FALSE),
         Z_nep0.04 = abs((slo_nep - -0.04)) / sqrt(se_nep^2),
         p_nep0.04 = pt(Z_nep0.04, 23, lower.tail = FALSE)) |>
  mutate(
    ...
    source = case_when(
      ...
      p_hco3dic < 0.05  & p_dic0.5 < 0.05 & p_nep0.04 > 0.05 ~ "HCO3",
      p_hco3dic < 0.05  & p_dic0.5 < 0.05 & p_nep0.04 < 0.05 & 
        between(slo_hco, -1.2, -0.8) & between(slo_nep, -0.06, -0.03)~ "HCO3",
      p_dic0.5 > 0.05 & r2_dic > 0.7 ~ "CaCO3",
      p_dic0.5 > 0.05 & r2_dic <= 0.7 & between(slo_nep, -0.2, -0.04) ~ "CaCO3",
      between(slo_nep, -0.2, -0.06) & between(slo_hco, -0.7, -0.4) ~ "CaCO3",
      TRUE ~ "CO2"
    )) 

df_example <- df_test |>
  filter(year > 2015, r2_dic > 0.8, r2_hco > 0.8, source != "CO2") |>
  group_by(source) |>
  slice_sample(n = 20) |>
  left_join(df)
