...
# hourly exO2 vs AT each day
df_mod_O2AT <- df |>
  drop_na(AT_mM, exO2) |>
  mutate(AT_uM = AT_mM * 1000) %>%
  group_by(year, month, date) |>
  filter(n() > 20) |>
  nest() |>
  mutate(mod = map(data, ~lm(.$exO2 ~ .$AT_uM)),
         tid = map(mod, broom::tidy),
         gl = map(mod, broom::glance))

# Regression results
df_O2AT <- ungroup(df_mod_O2AT) |>
  hoist(gl, "r.squared") |>
  select(-gl, -mod, -data) |>
  tidyr::unnest(cols = tid) |>
  filter(term == ".$AT_uM") |>
  select(date, slo_at = estimate, se_at = std.error, p_at = p.value, r2_at =r.squared)
...
# all results
df_all <- left_join(df_O2DIC, df_O2AT) |>
  left_join(df_CNEP)
